...
# SPPFT-identified safety band on Qwen3-1.7B (peak at block 18).
SAFETY_BLOCKS = {15, 16, 17, 18, 19}
N_LAYERS = 28  # Qwen3-1.7B has 28 transformer blocks
...
def build_target_modules():
    """Full-name target module list, excluding safety-band layers entirely."""
    targets = []
...
def encode_example(row, tokenizer, max_length):
    """Tokenize one row; mask labels on everything but the assistant target."""
    messages = [
...
def build_domain_weights(rows):
    """Equal-probability-per-domain sampler weights (1/N_domain per row)."""
...
def main():
    ...
    # NOTE: NOT modifying chat_template. Qwen3's default has enable_thinking
    # default true when not passed — that's what we want for mixed-SFT.
    ...
    # Spot-check the first item — confirm masking + chat template look right.
    sample = train_ds[0]
