...
def ties_merge(state_dicts, base_sd, weights, density):
    ...
    for i, key in enumerate(keys):
        ...
        # ELECT: per-param elected sign = sign of the sum
        stacked = torch.stack(tvs, dim=0)               # (N, *shape)
        elected = torch.sign(stacked.sum(dim=0))        # (*shape)

        # DISJOINT MERGE: average only entries whose sign matches the elect
...
def main():
    ...
    print(f"[merge] method={method}  n_models={len(models)}  weights={weights}")

    print("[merge] downloading specialty models...")
    model_dirs = [download_model(m["model"], args.cache_dir) for m in models]

    print("[merge] loading state dicts (CPU)...")
    sds = [load_state_dict(d) for d in model_dirs]
    ...
    if method == "linear":
        ...
        print(f"[merge] TIES density={density}, base={base_repo}")
    ...
    print(f"[merge] saving merged checkpoint -> {args.output_dir}")
